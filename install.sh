...
# Source conda directly
CONDA_BASE=$(conda info --base)
source "$CONDA_BASE/etc/profile.d/conda.sh"
conda activate "$ENV_NAME"
echo "🚀 Installing dependencies for Qwen3-VL-2B & Chart Extractor..."
pip install --upgrade pip
# Transformers >= 4.50 required for full Qwen3-VL support
pip install "transformers>=4.50" accelerate
pip install torch torchvision
# Core vision & data processing (includes chart extractor deps: opencv, numpy, scipy, pywt)
pip install pillow opencv-python numpy scipy PyWavelets
# Utilities & HF ecosystem
pip install pydantic tiktoken regex packaging huggingface-hub requests qwen-vl-utils
echo ""
